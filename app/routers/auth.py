...
def login(user_credentials : OAuth2PasswordRequestForm = Depends(), db : Session = Depends(database.get_db)):
   ...
   user = db.query(models.User, models.Email).join(models.Email, models.User.email_id == models.Email.id).filter(models.Email.email == user_credentials.username, models.User.email_id == models.Email.id).add_columns(models.User.id, models.User.password,  models.Email.email).first()
   if not user :
